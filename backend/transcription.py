...
def perform_stt(audio_path):
    """Convert speech to text with language detection and timestamps."""
    ...
    for idx, chunk_path in enumerate(chunk_paths):
        try:
            with sr.AudioFile(chunk_path) as source:
                ...
                detected_lang = detect_language(text)  # Detect language from text
                if text.strip():
                    timestamp = f"{idx * 60}:00"  # Assuming each chunk is 1 min
    ...
    return full_transcription if full_transcription else [{"time": "0:00", "type": "STT", "text": "No speech detected."}]
...
def process_video(file):
    ...
    final_transcription = merge_transcriptions(stt_text, ocr_text)
